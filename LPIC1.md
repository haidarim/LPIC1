...
**X:** The X Window System, commonly referred to as X11 or simply X, is a foundational component of graphical interfaces on Unix like OSs.

One of the hallmark features of X11 is its network transparency. This means that the X server (which manages display output and input devices) and the X clients (applications that request graphical resources) do not need to be on the same machine. For example, we can run a graphical application on a remote server, but display it on our local machine. This is accomplished by forwarding the graphical output over a network.

For example a remote application via SSH with X11 forwarding.

```sh
ssh -X <user@remote-server>
xclock  # This command runs the xclock application on the remote server but displays it locally
```

Client-Server Architecture: X11 operates on a client-server model:

- X Server: Manages display output and input devices. It handles drawing windows on the screen and processing user inputs (keyboard, mouse).

- X Clients: Applications that request graphical resources from the X server. Each client communicates with the X server to create windows, draw graphics, and respond to user inputs.

- Despite the names, the X Server runs on the machine displaying the graphics (local machine), while the X Clients could be local or remote applications.

Installation and Configuration: On most Linux distributions, X11 (specifically, the X.Org Server, which is the most widely used implementation of X11) is installed by default with desktop environments. However, if it is not installed, we can install it manually:
...
- Automatic Configuration: In most cases, X11 auto-detects hardware and configures itself, eliminating the need for manual configuration.

- Manual Configuration: Advanced users can manually configure X11 by editing configuration files.

Main Configuration File: /etc/X11/xorg.conf
Configuration Directory: /etc/X11/xorg.conf.d/

Example snippet to configure a specific screen resolution:
...
`~/.xsession-errors`: The ~/.xsession-errors file is a log file that captures errors and other output generated by X sessions (graphical login sessions). When we start an X session (like logging into our desktop environment), any errors, warnings, or diagnostic messages produced by X client applications or scripts during the session are written to this file.

`xhost`: It is a command used to manage access control to the X server. Specifically, it controls which users and hosts are allowed to connect to the X server to display graphical applications.

- Grant Access: We can allow a specific user or remote host to connect to our X server:

```sh
xhost +local:username # user name on local machine
xhost +inet:hostname
xhost +inet:192.168.1.10
```

- Revoke Access: We can also revoke access to the X server:
...
- Disabling Access Control (Insecure): This allows any user on any machine to connect to the X server, which is **INSECURE** and not recommended:
...
**DISPLAY:** It is an environment variable that tells X clients (graphical applications) which display server to connect to. The display server is responsible for handling all graphical output and input devices like the keyboard and mouse. The format of this variable is `hostname:displaynumber.screennumber`.

**xauth:** (X Authority) is a command-line utility used to manage the authorization information for X server connections. It controls access to the X server by managing the "magic cookies" that X clients (graphical applications) must present to the X server to establish a connection.

The primary method used by xauth is the MIT-MAGIC-COOKIE-1 scheme. When you start an X session, a unique cookie is generated and stored in a file (usually in ~/.Xauthority). Any X client that wants to connect to your X server must present this cookie to authenticate itself.

The `~/.Xauthority` file This file stores the authorization cookies for different displays. When you log into a graphical session, this file is created and managed automatically by the system to control access to the X server.

Basic commands:

- To list the contents of the `~/.Xauthority` use the list option:

```sh
xauth list
```

- To display all stored cookies along with the display they correspond to, for example:

```sh
hostname/unix:0  MIT-MAGIC-COOKIE-1  d0a1e6c7f7d2a8c92f5c8e13f8c9a9b3
```

- To Remove a specific authorization entery:

```sh
xauth remove hostname/unix:0
```

### Managing Users' and Groups' accounts (107.1)

**Passwords:**

- Changing Passwords: We can change password using the `passwd` command:

```sh
passwd
```

Note: The root user can change any user's password to any password without providing user's current password. Ex:
...
crontab might be not available by default on fedora (it used to be pre-installed on RHEL, Rocky Linux). But we have two alternative to schedule and scheduling custom services:

- **Using systemd timers:**

a. Creating a service: e.g. `/etc/systemd/system/my_service.service`:
...
b. Createa timer file: e.g `/etc/systemd/system/my_timer.timer`

```timer
[Unit]
Dscription=This will schedule for running every day at midnight
...
c. Enabling and statring the timer:

```sh
sudo systemdctl daemon-reexec
sudo systemctl enable --now my_timer.timer
```

-d Check existing timers by `sudo systemctl list-timers`

- **Installing crontab and cronie:**
  We can still use crontab:
...
- LC*ALL: Overrides all other LC*\* variables and LANG.
